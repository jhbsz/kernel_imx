...
#include <linux/ip.h>
#define L2HEADER_RESERVE (40)
#define HW_ETH_CHANNEL_BASE  10
#endif 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
#define RX_MAX_QUEUE_MEMORY (120 * 1518)
...
/*added by g00178026 for packet mirror , 2010-2-22*/
#ifdef CONFIG_USB_WAN_MIRROR
extern int cdc_map_to_lan_forward(struct sk_buff *skb);
/*
  取值范围0-4；0表示wan口数据镜像关闭;
  1-4表示开启，1表示eth0.2,2表示eth0.3，3表示eth0.4，4表示eth0.5
*/
static int cdc_wan_mirror = 0 ;
module_param(cdc_wan_mirror, int, S_IRUGO|S_IWUSR);
#endif
...
static unsigned int ncm_read_buf_count = 256;
module_param(ncm_read_buf_count, uint, S_IRUGO);

static unsigned short ncm_read_size_in1k = 16;
...
/* 控制，是否只使用内置的数据卡  */
static int only_use_inne_modem = 0;
module_param(only_use_inne_modem, bool, S_IRUGO|S_IWUSR);
 ...
 */
#define NCM_NTB_MIN_OUT_SIZE		(sizeof(struct usb_cdc_ncm_nth16) \
	+ sizeof(struct usb_cdc_ncm_ndp16) + 2 * NCM_NDP16_ENTRY_LEN)
...
#else
#define NCM_NTB_HARD_MAX_IN_SIZE ((u32)(max(2,ncm_read_size_in1k) * 1024))
#endif

#define RX_QLEN_NCM ncm_read_buf_count
#define TX_QLEN_NCM 64
...
/*  
    ...
    判断是否是b593的内置hub的第0个接口

*/
int hw_is_inne_modem(struct usb_device *xdev)
{
    /*  调试外置数据卡时候使用, 设置0的时候不过滤外置数据卡  */
    if(0 == only_use_inne_modem)
    {
        return 1;
    }
    ...
    if(NULL != xdev 
        && NULL != xdev->parent 
        && NULL != xdev->parent->parent 
        && xdev->parent->parent->parent == NULL
        && xdev->portnum == 1)
    {
        return 1;
    }
...
EXPORT_SYMBOL(hw_is_inne_modem);
...
void hw_skb_return (struct hw_cdc_net *dev, struct sk_buff *skb)
{
	int	status;
#ifdef CONFIG_USB_WAN_MIRROR
       /*added for packets mirror*/
       if(unlikely(cdc_wan_mirror))
       {
	...
	if(skb->len > 128)
	{
		...
		devdbg(dev,"hw_skb_return,len:%d receive sn:%x,  time:%ld-%ld",skb->len,sn,current_kernel_time().tv_sec,current_kernel_time().tv_nsec);
	}	
	else
	{
...
//add by w00178113
	...
	printk("\n");
	*/
	if ((PKT_DONE == blog_sinit(skb, skb->dev, TYPE_ETH, dev->channel, BLOG_USBPHY))) 
    {
		// printk("blog_sinit PKT_DONE**************************\n");
        return;
    }
#endif /* PKTFLOW */	
...
static int hw_change_mtu (struct net_device *net, int new_mtu)
{
    ...
    /*
    解决，MTU修改到1000后无法收到1500的包
    */
    if(dev->hard_mtu < net->mtu + net->hard_header_len){
...
static void rx_submit (struct hw_cdc_net *dev, struct urb *urb, gfp_t flags)
{
...
#ifdef PKTFLOW	
    if ((skb = alloc_skb (size + NET_IP_ALIGN + L2HEADER_RESERVE, flags)) == NULL) 
#else	
...
#endif
    {
        ...
        if(printk_ratelimit())
        {
            deverr(dev, "rx=%d, tx=%d, done=%d\n", dev->rxq.qlen, dev->txq.qlen, dev->done.qlen);
		}
...
#ifdef PKTFLOW	
    skb_reserve (skb, NET_IP_ALIGN+L2HEADER_RESERVE);
#else	
    skb_reserve (skb, NET_IP_ALIGN);
#endif
...
#ifdef CONFIG_USB_WAN_MIRROR
/*added by g00178026 for packets mirror  2011-2-22*/
int cdc_map_to_lan_forward(struct sk_buff *skb)
{
    struct net_device *dev = NULL;
    struct sk_buff *skb2 = NULL;

    /* xuweinan 修改适应我们的网口 */
    if(cdc_wan_mirror < 1 || cdc_wan_mirror > 2)
    {
        return 0;
    }
    ...
    if(skb2 == NULL)
    {
        printk("\r\n%s %d sdkb_clone failed",__FUNCTION__,__LINE__);
        return -1;
    }
    ...
    if(unlikely(!skb2->mac.raw))
    {
       printk("\nmap_to_lan_forward null\n");
       kfree_skb(skb2);
       return 0;
    }


    /* 获取镜像的lan口设备节点, 修改适应我们的网口 */
    switch(cdc_wan_mirror)
    {
      case 1:
          dev = __dev_get_by_name("eth0");
          break;
      case 2:
          dev = __dev_get_by_name("eth0.1");
          break;
#if 0          
      case 3:
          dev = __dev_get_by_name("eth0.4");
          break;
      case 4:
          dev = __dev_get_by_name("eth0.5");
          break;
#endif
      default:
    ...
    if(dev)
    {
        ...
        /*调用目的设备的发包函数，即bcm eth发包函数*/
        if((ret=dev->hard_start_xmit(skb2,dev)) != 0)
        {
            printk("\r\n%s %d et_tx error%d",__FUNCTION__,__LINE__,ret);
            return -1;
        }
        return 0;
    }
    ...
    {
        printk("\r\n%s %d didn't find mtp,g_mtp=%d", __FUNCTION__, __LINE__, cdc_wan_mirror);
        kfree_skb(skb2);
...
/*-------------------------------------------------------------------------*/
#ifdef PKTFLOW
static int hw_start_xmit (pNBuff_t pNBuff, struct net_device *net)
#else
...
#endif
{
    ...
    /* pktflow 没有控制包缓冲长度，需要在这里进行控制 */
    if (skb != NULL && netif_queue_stopped(net))
    {
        goto drop;
    }
    ...
    if(skb != NULL)
    {
	    blog_emit(skb, net, TYPE_ETH, dev->channel, BLOG_USBPHY);
    }
    if(skb != NULL && 60 == skb->len ){
        /*
        pktflow会将长度不足60的IP包，填充到60
        填充会导致部分路由器丢弃这些包
        */
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 25) //marui add 2010-7-8
...
for(i=0; i<3; i++)
{
	printk("cdc_ncm_config begin\n");
	printk("cdc_ncm_config enter %d\n", i);
    ...
    /*if(0 == first_flag++)
    {
        deverr(ctx->ndev, "first goto exit!\n");
		err = -EINVAL;
        goto exit;
    }
    
    deverr(ctx->ndev, "i=%d not first continue!\n",i);*/

	if (unlikely(!(ctx->formats & NTB_FORMAT_SUPPORTED_16BIT))) {
		deverr(ctx->ndev, "device does not support 16-bit mode time %d\n", i);
		err = -EINVAL;
		//goto exit;
		continue;
	}
	...
	if (unlikely(ctx->rx_max_ntb < NCM_NTB_MIN_IN_SIZE)) {
		deverr(ctx->ndev, "dwNtbInMaxSize (%u) must be at least "
			"%u time %d\n", ctx->rx_max_ntb, NCM_NTB_MIN_IN_SIZE, i);
		err = -EINVAL;
		//goto exit;
		continue;
	}
	...
	if (unlikely(ctx->tx_max_ntb < NCM_NTB_MIN_OUT_SIZE)) {
		deverr(ctx->ndev, "dwNtbOutMaxSize (%u) must be at least "
			"%u time %d\n", ctx->tx_max_ntb, NCM_NTB_MIN_OUT_SIZE, i);
		err = -EINVAL;
		//goto exit;
		continue;
	}
	...
	if (ncm_prefer_crc && (net_caps & NCM_NCAP_CRC_MODE)) {
		...
		err = usb_control_msg(udev, tx_pipe,
			...
			NCM_CONTROL_TIMEOUT);
		if (err < 0) {
			deverr(ctx->ndev, "failed setting crc-mode time %d\n", i);
			//goto exit;
			continue;
		}
	}
...
#define MIN_NDP_LEN(ndp_hdr_size, item_len) ((ndp_hdr_size) + \
	...
	struct ndp_parser_opts	*popts = &ctx->popts;
	struct sk_buff 		*skb2;
...
error2:
	dev_kfree_skb(skb2);
error:
    devwarn(dev,"cdc_ncm_rx_fixup error\n");
    return 0;
#undef NCM_BITS
#undef MIN_NDP_LEN
}
...
//	DECLARE_MAC_BUF(mac);
    ...
    //add by w00178113
#ifdef PKTFLOW
	net->hard_start_xmit = (HardStartXmitFuncP)hw_start_xmit;
#else /* PKTFLOW */
	...
	/*
	Huawei Ethernet Device %d
    */
    if(hw_is_inne_modem(xdev))
    {
        strcpy (net->name, "hed%d");
    }
    else
    {
...
#ifdef PKTFLOW
    dev->channel = HW_ETH_CHANNEL_BASE + udev->cur_altsetting->desc.bInterfaceNumber;
    printk("+++++++++++++++++++++++++++    pktflow channel %d \n", dev->channel);
#endif
	...
	if (0 < hw_send_tlp_download_request(udev)){
		...
		devdbg(dev, "%s: The tlp failed to activate", __FUNCTION__);		
	}
...
static inline int hw_get_ethernet_addr(struct hw_cdc_net *dev)
{
    printk("^^^^^^   dev:%d    interface:%d \n", dev->udev->devnum, dev->intf->cur_altsetting->desc.bInterfaceNumber);
	...
	/* add for apn */
    dev->net->dev_addr[4] = (unsigned char)(dev->udev->devnum);
    dev->net->dev_addr[5] = (unsigned char)(dev->intf->cur_altsetting->desc.bInterfaceNumber);
	return 0;
}
