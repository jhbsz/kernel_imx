...
static int imx_rt5633_init(struct snd_soc_pcm_runtime *rtd)
{
 ...
 */
static int __devinit imx_rt5633_probe(struct platform_device *pdev)
{
...
static int __devexit imx_rt5633_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
...
static struct platform_driver imx_rt5633_driver = {
	.probe = imx_rt5633_probe,
	.remove = imx_rt5633_remove,
	.driver = {
		   .name = "imx-rt5633",
		   .owner = THIS_MODULE,
		   },
...
static int __init imx_asoc_init(void)
{
	...
	ret = platform_driver_register(&imx_rt5633_driver);
	if (ret < 0)
		goto exit;

	imx_snd_device = platform_device_alloc("soc-audio", 7);
	if (!imx_snd_device)
...
err_device_alloc:
	platform_driver_unregister(&imx_rt5633_driver);
exit:
	return ret;
}
...
/* Module information */
MODULE_DESCRIPTION("ALSA SoC imx rt5633");
MODULE_LICENSE("GPL");
